...
> 버전: v1.0 | 작성일: 2026-05-23  
> 대상: 프론트엔드 개발자, 백엔드 개발자, UI/UX 디자이너(Figma 작업용)
...
3. [정보 구조도 (IA)](#3-정보-구조도-ia)
4. [디자인 시스템 & 가이드라인](#4-디자인-시스템--가이드라인)
5. [페이지별 기획 & UI 스펙](#5-페이지별-기획--ui-스펙)
   - [5-1. 로그인 / 회원가입 (Auth)](#5-1-로그인--회원가입-auth)
   - [5-2. 메인 / 검사 (Main / Test)](#5-2-메인--검사-main--test)
   - [5-3. 결과 (Result)](#5-3-결과-result)
   - [5-4. 궁합 (Compatibility)](#5-4-궁합-compatibility)
   - [5-5. 크레딧 충전 (Billing)](#5-5-크레딧-충전-billing)
   - [5-6. 마이페이지 / 히스토리 (My Page)](#5-6-마이페이지--히스토리-my-page)
6. [레이더 차트 렌더링 스펙](#6-레이더-차트-렌더링-스펙)
7. [API 연동 개요](#7-api-연동-개요)
...
| 핵심 기능 | 필기 이미지 업로드 → CNN 8지표 분석 → LLM 맞춤 리포트 생성 |
| 타겟 유저 | 심리 테스트에 흥미 있는 10~30대, MZ 세대 |
| 플랫폼 | 반응형 웹 (모바일 우선) |
| AI 엔진 | GraphoVision CNN (정확도 74.9%), LLM 멘트 생성 |

### 8가지 심리 지표

| # | 지표명 | 설명 |
|---|--------|------|
| 1 | Emotional Stability | 정서적 안정성 |
| 2 | Social Behavior | 사회적 행동 성향 |
| 3 | Mental Energy | 정신적 에너지 수준 |
| 4 | Willpower | 의지력 및 추진력 |
| 5 | Imagination | 상상력 및 창의성 |
| 6 | Fear / Anxiety | 불안 및 공포 성향 |
| 7 | Introversion / Extroversion | 내향/외향 성향 |
| 8 | Sensitivity | 감수성 및 민감도 |
...
| 20회권 | 20 cr | 14,000원 | 30% | 최대 할인 |

- 검사 1회 실행 시 **1 크레딧 자동 차감**
- 크레딧 잔액 0 → 검사 시도 시 충전 페이지로 이동 유도
- 충전 수단: 토스페이먼츠, 카카오페이 (간편결제 우선)
...
크레딧 잔액 확인
  ├── 잔액 있음 → 검사 진행
  └── 잔액 없음 → "크레딧이 부족합니다" 모달 → [충전하기] 버튼 → 충전 페이지
```
